...
func TestHeaderStep(t *testing.T) {
	...
	if err, _ := stages.StageLoopStep(m.Ctx, m.DB, nil, m.Sync, initialCycle, m.Log, m.BlockReader, nil); err != nil {
		t.Fatal(err)
	}
}
...
func TestMineBlockWith1Tx(t *testing.T) {
	...
	{ // Do 1 step to start txPool
		...
		initialCycle := stages.MockInsertAsInitialCycle
		if err, _ := stages.StageLoopStep(m.Ctx, m.DB, nil, m.Sync, initialCycle, log.New(), m.BlockReader, nil); err != nil {
			t.Fatal(err)
		}
	}
...
func TestAnchorReplace2(t *testing.T) {
	...
	if err, _ := stages.StageLoopStep(m.Ctx, m.DB, nil, m.Sync, initialCycle, m.Log, m.BlockReader, hook); err != nil {
		t.Fatal(err)
	}
}
...
func TestForkchoiceToGenesis(t *testing.T) {
	...
	require.NoError(t, err)
	defer tx.Rollback()
	err, _ = stages.StageLoopStep(m.Ctx, m.DB, tx, m.Sync, initialCycle, m.Log, m.BlockReader, nil)
	require.NoError(t, err)
	stages.SendPayloadStatus(m.HeaderDownload(), rawdb.ReadHeadBlockHash(tx), err, true)
